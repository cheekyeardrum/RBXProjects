if (game:GetService("CoreGui")):FindFirstChild("CHEEKY") and (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
    (game:GetService("CoreGui")).CHEEKY:Destroy()
...
ImageButton.MouseButton1Click:Connect(function()
    (game.CoreGui:FindFirstChild("CHEEKY")).Enabled = not (game.CoreGui:FindFirstChild("CHEEKY")).Enabled
end)
...
(getgenv()).LoadConfig = function()
    if readfile and writefile and isfile and isfolder then
        if not isfolder("CHEEKY") then makefolder("CHEEKY") end
        if not isfolder("CHEEKY/Library/") then makefolder("CHEEKY/Library/") end

        local path = "CHEEKY/Library/" .. game.Players.LocalPlayer.Name .. ".json"

        if not isfile(path) then
            writefile(path, game:GetService("HttpService"):JSONEncode(SettingsLib))
        else
            local Decode = game:GetService("HttpService"):JSONDecode(readfile(path))
            for i, v in pairs(Decode) do
                SettingsLib[i] = v
            end
        end
        print("Cheeky Library Loaded!")
    else
